...
class ThemeTest {
    ...
    /**
     * We use `createAndroidComposeRule` because the `Theme` composable interacts
     * with the Activity's `window` to set the status bar color.
     * `createComposeRule` does not provide access to an Activity.
     */
    @get:Rule
    ...
    fun lightTheme_appliesCorrectColorsAndSystemUi() {
        ...
        assertEquals("Status bar color should match light theme background", expectedStatusBarColor, window.statusBarColor)
    ...
    fun darkTheme_appliesCorrectColorsAndSystemUi() {
        ...
        assertEquals("Status bar color should match dark theme background", expectedStatusBarColor, window.statusBarColor)
