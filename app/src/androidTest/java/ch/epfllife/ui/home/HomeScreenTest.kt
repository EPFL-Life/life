 ...
 *
 * These tests verify that the main UI components of the home screen are displayed and that the
 * navigation actions are correctly triggered when the user interacts with the navigation bar.
 *
 * Assumptions:
 ...
 * - A BottomNavigation bar is present with items that have specific content descriptions for
...
class HomeScreenTest {
  ...
  fun setUp() {
    ...
    composeTestRule.setContent { Theme { HomeScreen(navigationActions = mockNavigationActions) } }
  }
  ...
  fun clickingAssociationsTab_navigatesToAssociationBrowser() {
    ...
    val associationsButton =
        composeTestRule.onNodeWithContentDescription("Navigate to Associations")
